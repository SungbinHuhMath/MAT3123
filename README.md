...
고정된 데이터셋에 의존하지 않고, 외부 API를 통해 **실시간 랜덤 이미지**를 수집합니다. 이는 모델이 학습 단계에서 보지 못한 새로운 데이터(Unseen Data)에 대해서도 얼마나 정확하게 특징을 잡아내는지(Generalization) 보여줍니다.

### 시각적 특징 추출 (Visual Feature Maps)
이미지의 단순한 픽셀 값을 넘어, CNN의 컨볼루션 레이어(Convolutional Layer)가 사물의 **윤곽선(Edge)**, **질감(Texture)**, **기하학적 문양**을 인지하는 과정을 시각화하여 딥러닝의 '블랙박스' 내부를 분석합니다.
